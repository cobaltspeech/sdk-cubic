...
public abstract class BaseCubicManager<T extends CubicAudioConfiguration> implements ICubicManager<T> {

    private final Context mContext;
    private final Lifecycle mLifecycle;

    private final Handler mMainThreadHandler;
    final OnCubicChangeListener mOnCubicChangeListener;

    private ManagedChannel mCubicChannel;
    private CubicGrpc.CubicStub mCubicService;

    final List<T> mCubicAudioConfigurations = new ArrayList<>();
    T mCubicAudioConfiguration;

    private final IAudioRecordController mIAudioRecordController;
    private final IConnectPreferenceManager mIConnectPreferenceManager;

    BaseCubicManager(@NonNull Context context,
                     @NonNull IConnectPreferenceManager iConnectPreferenceManager,
                     @NonNull OnCubicChangeListener listener,
                     Lifecycle lifecycle) {
        mContext = context;
        mIConnectPreferenceManager = iConnectPreferenceManager;
    ...
    public void connect() {
        ...
        if (mCubicService == null) {
            ...
            ConnectionConfiguration configuration = mIConnectPreferenceManager.getConnectionConfiguration();
            ...
            mCubicService.listModels(listModelsRequest, new StreamObserver<CubicOuterClass.ListModelsResponse>() {
                ...
                public void onNext(CubicOuterClass.ListModelsResponse value) {
                    mMainThreadHandler.post(() -> onCubicServiceStart(value));
                }
    ...
    protected abstract void onCubicServiceStart(@NonNull CubicOuterClass.ListModelsResponse value);
...
//        mCubicAudioConfigurations.clear();
    ...
    @Override
    public void setAudioConfiguration(T configuration) {
        mCubicAudioConfiguration = configuration;
    }

    @Override
    public List<T> getAudioConfigurations() {
        ...
        public void run() {
            ...
            CubicOuterClass.StreamingRecognizeRequest configs = CubicOuterClass.StreamingRecognizeRequest.newBuilder()
                    .setConfig(CubicOuterClass.RecognitionConfig.newBuilder()
                            .setModelId(mCubicAudioConfiguration.getCubicOuterModel().getId())
            ...
            CubicOuterClass.Model configuration = mCubicAudioConfiguration.getCubicOuterModel();
